...
contract LPPair is ERC20, Ownable {
    uint256 public constant MINIMUM_LIQUIDITY = 10 ** 3;
    bytes4 private constant SELECTOR = bytes4(keccak256(bytes('transfer(address,uint256)'))); // Taken from UNISWAP V2   
    address public lpRouterAddress;
    uint256 private reserveSpaceCoinToken;
    ...
    constructor(address _spaceCoinTokenAddress) ERC20("LP Token", "LPT") {
        lpRouterContract = new LPRouter(msg.sender, _spaceCoinTokenAddress);
    ...
    function _safeTransfer(address to, uint value) private {
        ...
        (bool success, bytes memory data) = address(spaceCoinTokenContract).call(abi.encodeWithSelector(SELECTOR, to, value));
    ...
    function _calculateAmountOut(uint256 _reserveTokenX, uint256 _reserveTokenY, uint256 _amountTokenX) private returns (uint256 _amountOutY) {
        ...
        uint256 fee = _reserveTokenX / 100; // 1% fee
        uint256 reserveTokenFee = k / (_reserveTokenX + _amountTokenX - fee);
    ...
    function swap(address _receiver) external lock isLPRouter returns (uint256 amountSpaceCoinTokenOutFees, uint256 amountETHFees) {
        ...
        require(amountETHOut < _reserveETH && amountSpaceCoinTokenOut < _reserveSpaceCoinToken, "LPPair: Output parameters less than reserve amount");

        if (amountSpaceCoinTokenOut > 0) {
            amountSpaceCoinTokenOutFees = _calculateAmountOut(_reserveETH, _reserveSpaceCoinToken, amountSpaceCoinTokenOut);
            reserveSpaceCoinToken = _reserveSpaceCoinToken - amountSpaceCoinTokenOutFees;
            _safeTransfer(_receiver, amountSpaceCoinTokenOutFees);
        }
        ...
        if (amountETHOut > 0) {
            ...
            (bool success, ) = address(_receiver).call{value: amountETHFees}("");
    ...
    function burn(address _receiver) external lock isLPRouter returns(uint256 amountSpaceToken, uint256 amountETH) {        
        ...
        totalETH -= amountETH;
        _safeTransfer(_receiver, liquidity);
